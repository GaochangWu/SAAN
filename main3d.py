...
# ----------------- Parameters setting -----------------------
dataset = 'ICME'    # ICME MPILF
sceneName = 'Development_dataset_6'  # Development_dataset_3 Bikes FairyCollection LivingRoom Mannequin WorkShop
sceneFile = '../Datasets/' + dataset + '/' # ../Datasets/ICME/  ../Datasets/MPILF/
modelPath = "./Model/model_SAANx3_shuffle"  # Model/


down_scale = 8
up_scale = down_scale
result_path = './Results/' + 'attSAAN/' + sceneName + 'x' + str(down_scale) + '/'
...
for i_iter in range(num_iter):
    ...
    if overlap == 1:
        for i in range(0, hei - batch[i_iter] + 1):
            slice3D = lf_in[i:i + batch[i_iter], :, :, :]
            slice3D = np.transpose(slice3D, (0, 3, 1, 2))
            slice3D = slice_reconstruction(wid, slice3D, ang_cur_out)

            if i == 0:
                lf_cur[i:i + batch[i_iter], :, :, :] = np.transpose(slice3D, (0, 2, 3, 1))
            else:
                lf_cur[i + batch[i_iter] // 2:i + batch[i_iter], :, :, :] = np.transpose(slice3D[batch[i_iter] // 2:],
                                                                                   (0, 2, 3, 1))
    else:
