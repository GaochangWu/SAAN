...
def pyramid_decompose(x):
    batch, hei, wid, chn = np.shape(x)
    x1 = tf.image.resize_bilinear(x, [hei, int(wid / 4)])

    temp_x1 = tf.image.resize_bilinear(x, [hei, int(wid / 2)])
    temp_x2 = tf.image.resize_bilinear(tf.image.resize_bilinear(x, [hei, int(wid / 4)]), [hei, int(wid / 2)])
    x2 = temp_x1 - temp_x2

    temp_x2 = tf.image.resize_bilinear(tf.image.resize_bilinear(x, [hei, int(wid / 2)]), [hei, wid])
    x3 = x - temp_x2
    return x1, x2, x3
